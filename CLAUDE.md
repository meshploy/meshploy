...
| `BUILD_CACHE_LIMIT_GB` | Each project's build cache is trimmed back under this after every build (default: `10`; `0` = no cap). The builder applies it, so it needs a builder image that knows `CACHE_LIMIT_MB` |
