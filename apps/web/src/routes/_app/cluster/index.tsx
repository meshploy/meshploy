...
function ProvisioningTokensPanel() {
  ...
  const { mutate: generate, isPending: generating } = useMutation({
    mutationFn: () =>
      nodesApi.createProvisioningToken(orgId!, NODE_ROLES.find((r) => r.value === role)!.label, null, token, role),
    onSuccess: (res) => {
  ...
  const agentPrompt = provToken
    ? [
        ...
        ``,
        `Look before you ask. Read my SSH config for hosts I already have:`,
        ``,
        `  grep -iE '^host ' ~/.ssh/config 2>/dev/null`,
        ``,
        `Then ask me one question, not four. If that listed candidates, show`,
        `them and ask which one this is, or whether it is a machine not in`,
        `there. If it listed nothing, ask for the address, the user, and the`,
        `port and key if they are not the defaults.`,
        ``,
        `The user is root unless I say otherwise. Check it works without`,
        `anything being typed, and stop if it does not:`,
        ``,
        `  ssh -o BatchMode=yes -o ConnectTimeout=8 <host> 'hostname; id -u'`,
        ``,
        `BatchMode is not optional: without it a server that wants a password`,
        `leaves the command waiting for input you cannot give, which looks to me`,
        `like you are still working. If it fails, do not ask me for a password`,
        `and do not use sshpass - tell me, and I will fix the access.`,
        ``,
        `If id -u was not 0, check that account can become root silently with`,
        `\`sudo -n true\`, and stop and tell me if it cannot.`,
        ``,
        `## Check it is not already spoken for`,
        ``,
        ``,
