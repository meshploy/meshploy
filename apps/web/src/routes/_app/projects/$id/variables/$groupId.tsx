...
function ItemRow({ item, onDelete, isDeleting }: { item: ApiVariableGroupItem; onDelete: () => void; isDeleting: boolean }) {
  ...
  return (
    <div className="rounded-md border border-border/60 bg-muted/5 px-3 py-2.5 flex items-center gap-3">
      ...
      {item.is_secret && (
        <span className="text-[11px] font-medium uppercase tracking-wider px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20 shrink-0">secret</span>
      )}
...
function ServiceRow({ svc, onDetach, isDetaching }: { svc: ApiService; onDetach: () => void; isDetaching: boolean }) {
  return (
    <div className="rounded-md border border-border/60 bg-muted/5 px-3 py-2.5 flex items-center gap-3">
      ...
      {svc.status !== "running" && (
        <span className="text-[11px] font-medium uppercase tracking-wider px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20 shrink-0">not deployed</span>
      )}
    ...
    <div className="flex flex-col min-h-full">
      <DetailPageHeader
        ...
        badge={group.system_managed
          ? (
            <span className="flex items-center gap-1 text-[11px] font-medium uppercase tracking-wider px-1.5 py-0.5 rounded bg-muted text-muted-foreground border border-border/60">
      ...
      <div className="console-page space-y-6">
        ...
        <div className="resource-metrics">
          <MetricTile icon={Layers} label="Variables" value={group.items.length} detail="Keys in this group" />
          <MetricTile icon={Lock} label="Management" value={<span className="text-xl">{group.system_managed ? "Automatic" : "Custom"}</span>} detail={group.system_managed ? "Generated by the service" : "Managed by your team"} />
          <MetricTile icon={Server} label="Consumers" value={group.system_managed ? "-" : attachedServices.length} unit={group.system_managed ? undefined : "services"} detail={group.system_managed ? "Service-managed group" : "Receive values on next deploy"} />
        </div>
