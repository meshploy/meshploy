...
function serviceTerminalWsUrl(orgId: string, projectId: string, serviceId: string, podName: string, ticket: string): string {
  const apiBase =
  ...
  return `${base}/api/v1/orgs/${orgId}/projects/${projectId}/services/${serviceId}/pods/${encodeURIComponent(podName)}/terminal?ticket=${encodeURIComponent(ticket)}`
}
...
export function ServiceTerminal({ payload }: { payload: ServiceTerminalPayload }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      ws = new WebSocket(
        serviceTerminalWsUrl(orgId, payload.projectId, payload.serviceId, payload.podName, ticket)
      )
