...
export const demoDeployment = {
  ...
  source_commit_message: "Rate-limit the login endpoint",
  deployed_at: DEMO_NOW,
  created_at: DEMO_NOW,
  updated_at: DEMO_NOW,
}
...
// What else the demo gateway runs: a compose project, a container on the host's
// network, and a stopped one - the three shapes the section has to read well.
export const demoHostContainers = {
  available: true,
  runtime: "docker",
  version: "27.3.1",
  checked_at: DEMO_NOW,
  ...
  containers: [
    {
      ...
      started_at: "2026-06-15T09:12:04Z",
      ports: [{ host_ip: "127.0.0.1", host_port: 3001, port: 3000, protocol: "tcp" }],
      network_mode: "monitoring_default",
      volumes: ["monitoring_grafana-data"],
    ...
    },
    {
      ...
      restart_count: 2,
      created_at: "2026-06-15T09:12:00Z",
      started_at: "2026-06-15T09:12:02Z",
      ports: [{ host_ip: "127.0.0.1", host_port: 9090, port: 9090, protocol: "tcp" }],
      network_mode: "monitoring_default",
      bind_sources: ["/srv/monitoring/prometheus.yml"],
      volumes: ["monitoring_prom-data"],
      compose_project: "monitoring",
    ...
    },
    {
      ...
      network_mode: "host",
      bind_sources: ["/var/lib/tailscale"],
      memory_mb: 38,
      cpu_percent: 0.2,
      host_network: true,
    ...
    },
    {
      ...
      network_mode: "bridge",
      bind_sources: ["/srv/backups"],
      host_network: false,
      kind: "standalone" as const,
    },
  ],
  groups: [{ name: "monitoring", kind: "compose" as const, count: 2, running: 2, host_network: false }],
}

// The endpoints those containers and the host itself hold. Covers every shape
// the page has to read: a container's published port, a container on the host's
// network, a plain host process, one bound to loopback, one already routed, and
// a port nothing listens for because the runtime forwards it in the kernel.
  ...
  nodes: [
    {
      ...
      hidden: 9,
      ignored: 0,
      containers: demoHostContainers.containers,
      endpoints: [
        {
          id: "gw:0.0.0.0:22", address: "0.0.0.0", port: 22, protocol: "tcp",
          process: "sshd", label: "SSH", http: false,
          source: "listener" as const, scope: "all" as const, routable: true, suggested_port: 22,
        },
        {
          id: "gw:0.0.0.0:443", address: "0.0.0.0", port: 443, protocol: "tcp",
          process: "tailscale", label: "HTTPS", http: true, tls: true,
          source: "listener" as const, scope: "all" as const, routable: true,
          container: { id: "e3c4d5e6f7a8", name: "tailscale-exit", image: "tailscale/tailscale:v1.76.1", host_network: true },
        },
        {
          id: "gw:127.0.0.1:3001", address: "127.0.0.1", port: 3001, protocol: "tcp",
          process: "docker-proxy", label: "HTTP", http: true,
          source: "listener" as const, scope: "host" as const, routable: true, suggested_port: 3001,
          container: { id: "c1a2b3c4d5e6", name: "monitoring-grafana-1", image: "grafana/grafana:11.2.0", compose_project: "monitoring" },
          routed: [{ kind: "http" as const, id: DEMO_ROUTE_ID, name: "grafana.demo.meshploy.app" }],
        },
        {
          id: "gw:127.0.0.1:9090", address: "127.0.0.1", port: 9090, protocol: "tcp",
          process: "docker-proxy", label: "HTTP", http: true,
          source: "listener" as const, scope: "host" as const, routable: true, suggested_port: 9090,
          container: { id: "d2b3c4d5e6f7", name: "monitoring-prometheus-1", image: "prom/prometheus:v2.54.1", compose_project: "monitoring" },
        },
        {
          id: "gw:0.0.0.0:5433", address: "0.0.0.0", port: 5433, protocol: "tcp",
          label: "PostgreSQL", http: false,
          source: "published" as const, scope: "all" as const, routable: true, suggested_port: 5433,
          container: { id: "aa11bb22cc33", name: "legacy-db", image: "postgres:15", compose_project: "legacy" },
        },
        {
          id: "gw:0.0.0.0:8096", address: "0.0.0.0", port: 8096, protocol: "tcp",
          process: "docker-proxy", http: true, label: "HTTP",
          source: "listener" as const, scope: "all" as const, routable: true, suggested_port: 8096,
          via_runtime: true,
        },
        {
          id: "gw:127.0.0.1:11434", address: "127.0.0.1", port: 11434, protocol: "tcp",
          process: "ollama", http: false,
          source: "listener" as const, scope: "host" as const, routable: true, suggested_port: 11434,
        },
      ],
    },
  ],
