...
| POST | `/system/migrate/dokploy/{kind}` | ✓ | Instance owner: ask the host agent to `detect` Dokploy or `plan` moving it (read-only on the host); 409 when the agent is not reporting |
